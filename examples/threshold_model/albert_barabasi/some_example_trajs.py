...
def main():
    ...
    x_init = np.random.randint(0, 2, size=(4, params.num_agents))
    cv = OpinionShares(2, True, idx_to_return=0)
    x_anchor = np.load("data/x_data.npz")["x_anchor"]

    t, x = sample_many_runs(params, x_init, 30, 500, num_samples, -1, collective_variable=cv)
    # t, x = sample_many_runs(params, x_anchor[[10, 67, 390, 512], :], 30, 500, num_samples, -1, collective_variable=cv)
